...
describe('generateAIContextFiles', () => {
  ...
  it('keeps the CLAUDE.md GitNexus block under the token-cost budget (#856)', async () => {
    // The pre-trim block was ~5465 chars. After #856 it dropped to roughly half.
    // The ceiling is a soft cap that still leaves headroom for legitimate future
    // additions but will fail loudly if the trim is reverted or someone pads the
    // block back out toward the original size.
    //
    // Raised 2700 → 2900 for #243: the regression-compare example (one
    // load-bearing per-repo `base_ref` line on the detect_changes bullet).
    //
    // Raised 2900 → 3050 for the security-findings remediation (R4): a one-line
    // "auto-generated from untrusted repository symbols" provenance comment is
    // prepended to the block. It is a load-bearing security note (the block is
    // generated from untrusted repo symbol names), not a revert of the trim —
    // the block stays well under half the original size.
    const stats = { nodes: 50, edges: 100, processes: 5 };
    await generateAIContextFiles(tmpDir, storagePath, 'TestProject', stats);
    ...
    expect(block.length).toBeLessThan(3050);
  });
...
`;
  ...
  // ──────────────────────────────────────────────────────────────────
  // R4 — generated-skill rows escape untrusted graph-derived labels
  // ──────────────────────────────────────────────────────────────────

  it('escapes a pipe in a generated-skill label so it cannot inject a table row (R4)', () => {
    const content = generateGitNexusContent('P', { nodes: 1, edges: 1, processes: 1 }, [
      {
        name: 'evil-area',
        label: 'Evil | Injected | Row',
        symbolCount: 9,
        fileCount: 2,
      },
    ]);
    // The CLI skills table is a 2-column table, so a legitimate row has exactly
    // 3 structural (unescaped) pipes. The in-label pipes are escaped, so the
    // hostile label does not add columns.
    const row = content.split('\n').find((l) => l.includes('Evil'))!;
    expect(row).toBeDefined();
    const structuralPipes = (row.match(/(?<!\\)\|/g) || []).length;
    expect(structuralPipes).toBe(3);
    // The raw injected fragment never appears unescaped.
    expect(content).not.toContain('Evil | Injected | Row');
  });

  it('a newline in a generated-skill label cannot inject a new table row (R4)', () => {
    const content = generateGitNexusContent('P', { nodes: 1 }, [
      {
        name: 'nl-area',
        label: 'Real\n| Fake Row | x |',
        symbolCount: 3,
    ...
    expect(content.split('\n')).not.toContain('| Fake Row | x |');
  });

  it('includes the auto-generated-from-untrusted-symbols note in the block (R4)', () => {
    const content = generateGitNexusContent('P', { nodes: 1, edges: 1, processes: 1 });
    expect(content.toLowerCase()).toContain('auto-generated by gitnexus');
    expect(content.toLowerCase()).toContain('untrusted');
  });

  it('the auto-generated note keeps the CLAUDE.md block under the #856 budget (R4)', async () => {
    const stats = { nodes: 50, edges: 100, processes: 5 };
    const subDir = await fs.mkdtemp(path.join(os.tmpdir(), 'gn-r4-budget-'));
    const subStorage = path.join(subDir, '.gitnexus');
    await fs.mkdir(subStorage, { recursive: true });
    try {
      await generateAIContextFiles(subDir, subStorage, 'TestProject', stats);
      const content = await fs.readFile(path.join(subDir, 'CLAUDE.md'), 'utf-8');
      const block = content.slice(
        content.indexOf('<!-- gitnexus:start -->'),
        content.indexOf('<!-- gitnexus:end -->'),
      );
      // Ceiling raised to 3050 to absorb the R4 provenance note (see #856 budget
      // test rationale).
      expect(block.length).toBeLessThan(3050);
    } finally {
      await fs.rm(subDir, { recursive: true, force: true });
